...
const Row = memo(({ item, fetchData, startIndex, index, dataCountries }) => {
	...
	const { mutate: deleteNationality, isPending } = useDeleteNationality();

	const handleDelete = () => {
		deleteNationality(item.id, {
			onSuccess: () => {
				toaster.create({
					title: 'Nacionalidad eliminado correctamente',
					type: 'success',
				});
				fetchData();
				setOpen(false);
...
export const SettingsNationalityTable = ({
	data,
	fetchData,
	dataCountries,
}) => {
				...
				<Table.Root size='sm' w='full' striped>
					<Table.Header>
						<Table.Row bg={{ base: 'its.100', _dark: 'its.gray.400' }}>
							...
							<Table.ColumnHeader>Pais</Table.ColumnHeader>
...
SettingsNationalityTable.propTypes = {
