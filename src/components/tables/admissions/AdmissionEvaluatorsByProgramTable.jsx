...
const Row = memo(
	({ item, fetchData, isLoading, startIndex, index, data, sortConfig }) => {
		const hasEvaluations = item.evaluations && item.evaluations.length > 0;
		const isEvaluated =
			hasEvaluations &&
			item.evaluations.every(
				(e) => e.status_qualification_display === 'Completado'
			);
		const averageScore = hasEvaluations
			? (
					item.evaluations.reduce(
						(sum, evalItem) => sum + parseFloat(evalItem.qualification || 0),
						0
		...
		const onSubmitQualification = ({
			...
			const validNote =
				/^(\d{1,2})(\.\d{1,2})?$/.test(qualification) &&
				parseFloat(qualification) >= 0 &&
				parseFloat(qualification) <= 20;
			...
			updateExamGrade(
				{
					uuid: selectedEvaluations.uuid,
					payload: { qualification, feedback },
				},
				{
					onSuccess: () => {
						fetchData();
		...
		return (
			<Table.Row key={item.id} bg={index % 2 === 0 ? 'gray.100' : 'white'}>
				...
				<Table.Cell>
					<Badge
						colorPalette={isEvaluated ? 'green' : 'orange'}
						fontWeight='semibold'
					>
