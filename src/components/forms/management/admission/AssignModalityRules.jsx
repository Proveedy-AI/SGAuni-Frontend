import { Checkbox, Field, Modal, toaster } from '@/components/ui';
import { useAssignModalityRules, useReadModalityRules } from '@/hooks';
import { Flex, IconButton, Spinner, Stack, VStack } from '@chakra-ui/react';
              ...
              <VStack align='start'>
                {dataModalityRules?.results?.map((rule) => (
                  <Field key={rule.id} orientation='horizontal'>
                    <Checkbox checked={selectedRuleIds.includes(rule.id)} onChange={(e) => handleCheckboxChange(rule.id, e.target.checked)}>
                      {rule.field_name}
                    </Checkbox>
                  </Field>
                ))}
