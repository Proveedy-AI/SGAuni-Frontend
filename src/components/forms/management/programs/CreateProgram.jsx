...
export const AddProgram = ({ fetchData, programTypesOptions, coordinatorsOptions, loadingProgramTypes, loadingCoordinators }) => {
  ...
  const handleSubmitData = async (e) => {
    e.preventDefault();
    if (!programRequest.name || !programRequest.type || !programRequest.coordinator || programRequest.price_credit <= 0) return;
      ...
      <Stack css={{ '--field-label-width': '120px' }}>
        <Field label='Nombre del Programa'>
          <Input
            ...
            onChange={(e) => setProgramRequest({ ...programRequest, name: e.target.value })}
          />
        </Field>
        ...
        <Field label='Precio por crédito (s/)'>
          <Input
            ...
            onChange={(e) => setProgramRequest({ ...programRequest, price_credit: e.target.value })}
          />
        </Field>
      </Stack>
    </Modal>
