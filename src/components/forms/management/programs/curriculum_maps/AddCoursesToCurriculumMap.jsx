...
import { useBulkCreateCurriculumMap, useReadCurriculumMapsCourses } from '@/hooks/curriculum_maps_courses';
import { Box, Button, Card, Flex, Heading, IconButton, Input, SimpleGrid, Stack } from '@chakra-ui/react';
...
export const AddCoursesToCurriculumMap = ({ item }) => {
  ...
  const { mutate: addCoursesToCurriculum, isPending } = useBulkCreateCurriculumMap();
  const {
  ...
  const handleAddOrUpdateCourse = () => {
    ...
    if (editIdx !== null) {
      setCoursesList((prev) => prev.map((c, i) => (i === editIdx ? { ...form } : c)));
    } else {
  ...
  const validateForm = () => {
    ...
    if (form.cycle === '' || isNaN(Number(form.cycle))) err.cycle = 'Ciclo requerido';
    if (form.credits === '' || isNaN(Number(form.credits))) err.credits = 'Créditos requeridos';
    if (!Array.isArray(form.prerequisite_ids)) err.prerequisite_ids = 'Selecciona los prerrequisitos';
    setErrors(err);
  ...
  const handleSubmit = () => {
    ...
    addCoursesToCurriculum(payload, {
      ...
      onError: (error) => {
        toaster.create({
          title: 'Error',
          description: error?.response?.data?.message || 'Ocurrió un error al asignar los cursos.',
          type: 'error',
        });
      }
    });
      ...
      <Stack>
        <Card.Root>
          ...
          <Card.Body maxH={{ base: '90vh', md: '600px' }} overflow="hidden">
            <SimpleGrid columns={{ base: 1, md: 2 }} gap={4} mb={4} height="100%">
              ...
              <Stack gap={2} border="1px solid #e2e8f0" borderRadius="md" p={3}>
                ...
                >
                  <ReactSelect
                    ...
                    value={courseOptions.find(opt => opt.value === form.course_id) || null}
                    onChange={opt => handleFormChange('course_id', opt ? opt.value : null)}
                ...
                <Field label="Obligatorio">
                  <Checkbox
                    ...
                    onChange={e => handleFormChange('is_mandatory', e.target.checked)}
                  >
                    Es obligatorio
                  </Checkbox>
                </Field>
                ...
                >
                  <ReactSelect
                    options={prerequisiteOptions.filter(opt => opt.value !== form.course_id)}
                    value={prerequisiteOptions.filter(opt => form.prerequisite_ids.includes(opt.value))}
                    onChange={opts => handleFormChange('prerequisite_ids', opts ? opts.map(o => o.value) : [])}
                    isMulti
                    isClearable
                    isSearchable
                ...
                <Stack direction="row" gap={2}>
                  ...
                  {editIdx !== null && (
                    <Button colorPalette="gray" variant="outline" onClick={resetForm}>
                      Cancelar
                    </Button>
                  )}
                </Stack>
              ...
              <Box
                ...
                bg="white"
              >
                <Heading size="sm" mb={2} color="gray.600">Cursos agregados</Heading>
                {coursesList.length === 0 ? (
                  <Box color="gray.400">No hay cursos agregados.</Box>
                ) : (
                  <Stack gap={2}>
                    {coursesList.map((c, idx) => {
                      const courseLabel = dataCourses?.results?.find(dc => dc.id === c.course_id)?.name || '';
                      return (
                        <Card.Root key={idx} border="1px solid #e2e8f0" borderRadius="md">
                          <Flex justify="space-between" align="center" p={2}>
                            <Box>
                              <b>{courseLabel}</b> | Ciclo: {c.cycle} | Créditos: {c.credits} | {c.is_mandatory ? 'Obligatorio' : 'Opcional'}
                              <br />Prerrequisitos: {c.prerequisite_ids.map(pid => {
                                const pre = dataCourses?.results?.find(dc => dc.id === pid);
                                return pre ? pre.name : '';
                              }).join(', ') || 'Ninguno'}
                            </Box>
                            <Stack direction="row" gap={1}>
                              <IconButton size="xs" colorPalette="yellow" onClick={() => handleEditCourse(idx)}>
                                <BsPencil />
                              </IconButton>
                              <IconButton size="xs" colorPalette="red" onClick={() => handleRemoveCourse(idx)}>
                                <BsTrash />
                              </IconButton>
                            </Stack>
                          </Flex>
