...
export const LoadEvaluationsByExcelModal = ({
	dataCourseGroup,
	fetchData,
	fetchGradesReport,
}) => {
	...
	const { mutate: generateTemplate, isPending: loadingTemplate } =
		useReadExcelTemplate();
	const { mutate: uploadEvaluations, isPending: loadingUpload } =
		useUploadEvaluationsByExcel();
	...
	const handleDownloadTemplate = () => {
		...
		generateTemplate(dataCourseGroup?.id, {
			onSuccess: (data) => {
        ...
        console.log(error)
				toaster.create({
					...
					description: error.message, // ← ya sale "Curso no encontrado o sin permisos"
					type: 'error',
				});
			},
		});
	...
	const handleUploadEvaluations = async () => {
		...
		try {
			...
			uploadEvaluations(
				{ courseGroupId: dataCourseGroup.id, payload },
				{
					onSuccess: () => {
						toaster.create({
							title: 'Evaluaciones cargadas',
							description:
								'Las evaluaciones del Excel han sido procesadas correctamente',
							type: 'success',
						});
						setOpen(false);
						setExcelFile(null);
	...
	const isUploadDisabled =
		!excelFile || !acceptedWarning || isLoading || loadingUpload;
			...
			<Stack gap={5}>
				...
				>
					<VStack align='start' spacing={1}>
						<Text>
							• Las evaluaciones cargadas desde Excel{' '}
							<strong>sobrescribirán</strong> todas las notas que hayan sido
							ingresadas manualmente.
						</Text>
						<Text>• Esta acción no se puede deshacer una vez completada.</Text>
						<Text>
							• Asegúrese de que el archivo Excel contiene las evaluaciones
				...
				>
					...
					>
						<Text fontSize='sm' fontWeight='medium' color='orange.800'>
							He leído y entiendo que las notas del Excel sobrescribirán las
							notas manuales existentes
						</Text>
					</Checkbox>
				</Box>
				...
				>
					<VStack align='start' spacing={1} mt={2}>
						...
						<Text fontSize='sm'>
							• No modifique las columnas de identificación
						</Text>
						<Text fontSize='sm'>
							• Ingrese solo valores numéricos en las calificaciones
						</Text>
					</VStack>
				</Alert>
			</Stack>
