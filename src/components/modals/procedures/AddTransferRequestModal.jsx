...
export const AddTransferRequestModal = ({
	user,
	available,
	loading,
	dataMyPrograms,
	...
	const { mutate: createTransferRequest, isPending } =
	...
	const validateFields = () => {
		...
		if (programFrom && programTo && programFrom.value === programTo.value) {
			newErrors.programTo =
				'El programa de destino debe ser diferente al de origen';
		}
			...
			createTransferRequest(payload, {
				onSuccess: () => {
					toaster.create({
						...
						description:
							'Tu solicitud de traslado ha sido enviada exitosamente.',
						type: 'success',
					});
					setOpen(false);
					setProgramFrom(null);
	...
	const programToOptions =
		dataPrograms?.results
			?.filter((program) => !myProgramIds.includes(program.id))
			.map((program) => ({
				value: program.id,
				label: program.name,
			})) || [];
			...
			<VStack spacing={6} align='stretch' ref={contentRef}>
				...
				>
					...
					<Text fontSize='xs' color='blue.600'>
						El traslado interno le permite cambiar de un programa a otro dentro
						de la universidad. Una vez enviada la solicitud, será evaluada por
					...
					<HStack justify='space-between' align='center'>
						<Box flex='1'>
							...
							<Text fontSize='xs' color='green.600'>
								Descarga y completa la plantilla oficial para adjuntar con tu
				...
				<Field label='Solicitud:'>
					<CompactFileUpload
						name='document_path'
						accept='.pdf'
						onChange={(file) => setDocumentPath(file)}
				...
				>
					...
					<Text fontSize='xs' color='amber.600'>
						• La solicitud será evaluada por el área académica correspondiente
						<br />
						• El proceso puede tomar entre 5 a 10 días hábiles
						<br />
						• Una vez aprobado, no podrá revertir el traslado
						<br />• Debe cumplir con los requisitos académicos del programa
						destino
					</Text>
				</Box>
			</VStack>
