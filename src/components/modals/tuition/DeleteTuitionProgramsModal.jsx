...
export const DeleteTuitionProgramsModal = ({
	item,
	fetchData,
}) => {
	const { mutate: deleteEnrollmentsPrograms, isPending } =
		useDeleteEnrollmentsPrograms();
				...
				<Box>
					...
					>
						<IconButton
							colorPalette='red'
							size='xs'
							disabled={item.status === 4 || item.status === 2} // || !permissions?.includes('enrollments.myprograms.delete')}
						>
							<FiTrash2 />
						</IconButton>
					</Tooltip>
