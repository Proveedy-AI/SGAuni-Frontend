...
export const PaymentOrdersView = () => {
	...
	const filteredPaymentOrdersByRequest = dataPaymentOrders?.results?.filter(
		(order) =>
			(!searchValue.id_orden ||
				order.id_orden?.includes(searchValue.id_orden)) &&
			(!searchValue.document_num ||
				order.document_num?.includes(searchValue.document_num)) &&
			(!searchValue.email || order.email?.includes(searchValue.email)) &&
			(!searchValue.due_date || order.due_date === searchValue.due_date)
	);

	const sortedPaymentOrders = filteredPaymentOrdersByRequest?.sort(
