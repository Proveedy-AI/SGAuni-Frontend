...
export const PaymentRequestsView = () => {
	...
	const filteredPaymentRequests = dataPaymentRequests?.results?.filter(
		(item) =>
			(!searchValue.purpose_display ||
				item?.purpose_display
					.toLowerCase()
					.includes(searchValue.purpose_display.toLowerCase())) &&
			(searchValue.document_type.value === 0 ||
				item?.document_type === searchValue.document_type.value) &&
			(searchValue.payment_method.value === 0 ||
				item?.payment_method === searchValue.payment_method.value) &&
			(searchValue.status.id === 0 ||
				item?.status === searchValue.status.value) &&
			(!searchValue.requested_at ||
				format(parseISO(item?.requested_at), 'yyyy-MM-dd') ===
					format(parseISO(searchValue.requested_at), 'yyyy-MM-dd'))
	);
			...
			>
				<InputGroup flex='1' startElement={<FiSearch />}>
					<Input
						...
						onChange={(e) =>
							setSearchValue({ purpose_display: e.target.value })
						}
					/>
				</InputGroup>
				<LinkButton
					bg='#711610'
					href='./downloads/plantilla_cobranzas_demo.xlsx'
					download={''}
				>
					Descargar Plantilla
				</LinkButton>
