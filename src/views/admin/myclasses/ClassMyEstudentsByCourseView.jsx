...
import { Box, Card, Heading, Text, Table, Input, Flex, Stack, SimpleGrid, Icon, IconButton, Badge } from "@chakra-ui/react";
...
export const ClassMyEstudentsByCourseView = () => {
  ...
  const filteredStudents = dataStudentsLocal.results.filter(student => {
    const matchesName = student.student_name.toLowerCase().includes(filteredName.toLowerCase());
    const matchesStatus = selectedStatus ? student.qualification_status === selectedStatus.value : true;
      ...
      <Card.Root mb={6}>
        <Card.Header>
          ...
          <Text fontSize="sm" color="gray.500">
            Código: {dataCourseGroup?.course_code} | Grupo: {dataCourseGroup?.group_code}
          </Text>
        </Card.Header>
        <Card.Body>
          <Text>Docente: {dataCourseGroup?.teacher_name}</Text>
          <Text>Capacidad: {dataCourseGroup?.capacity}</Text>
          <Text>Periodo de inscripción: {dataCourseGroup?.enrollment_period_program_course}</Text>
        </Card.Body>
      </Card.Root>
        ...
        <Card.Header>
          <Flex justify='space-between' align='center'>
            <Flex align='center' gap={2}>
              ...
              <Heading fontSize='24px'>Estudiantes del curso</Heading>
            </Flex>
              ...
              <Table.Row bg={{ base: 'its.100', _dark: 'its.gray.400' }}>
                ...
                {has_configured && (
                    ...
                    {evaluations.map(evaluation => (
                      <Table.Cell key={evaluation.id}>{evaluation.name}</Table.Cell>
                    ))}
