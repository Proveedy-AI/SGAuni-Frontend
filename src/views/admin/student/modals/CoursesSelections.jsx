...
export const CoursesSelections = ({ dataCoursesByPeriod }) => {
						...
						<Box mb={3}>
							...
							>
								<Text fontSize={14} fontWeight='bold' color='purple.700'>
									PERIODO ACADÉMICO {dataCoursesByPeriod.academic_period}
								</Text>
							</Box>
							...
							<Box my={2} px={6} py={3} bg={summaryBg} borderRadius='md'>
								<HStack justify='space-between'>
									<Text fontSize='sm' color='gray.600'>
										...
										{dataCoursesByPeriod.total_courses}
									</Text>
									<Text fontSize='sm' color='gray.600'>
										<strong>Total Créditos:</strong>{' '}
										{dataCoursesByPeriod.courses.reduce(
							...
							>
								<Table.ScrollArea>
									<Table.Root variant='simple' size='sm'>
										...
										<Table.Body>
											{dataCoursesByPeriod.courses.map((course, index) => (
												<Table.Row
													key={index}
													_hover={{ bg: hoverBg }}
													borderColor={borderColor}
