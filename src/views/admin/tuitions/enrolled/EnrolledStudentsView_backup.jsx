import { Encryptor } from "@/components/CrytoJ  const enrollmentProcessName = !isLoadingEnrollmentProcess ? dataEnrollmentProcess?.academic_period_name : 'Cargando...';
  const courseGroupName = !isLoadingCourseGroup ? dataCourseGroup?.course_name : 'Cargando...';

  // Filtrar estudiantes
  const filteredStudents = dataEnrollmentReport?.[0]?.enrolled_students?.filter(ptor";
...
export const EnrolledStudentsView = () => {
  ...
  const { enrolled_students, ...courseGroup } = dataEnrollmentReport?.[0] || {};
