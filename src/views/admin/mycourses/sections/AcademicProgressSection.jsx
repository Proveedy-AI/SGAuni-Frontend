...
export const AcademicProgressSection = ({ academicProgress, isLoading }) => {
	...
	return (
		<Card.Root w={'full'} mx='auto' bg='white' borderRadius='lg'>
			<Card.Header
				py={4}
				bg='blue.50'
				borderBottom='1px solid'
			...
			<Card.Body p={4} gap={4}>
				<Card.Root p={4} borderRadius='md' overflow='hidden'>
					<SimpleGrid columns={{ base: 1, lg: 2 }} fontSize={'sm'}>
						<Text color='gray.500'>
							Programa: <b>{program?.name}</b>
						</Text>
						<Text color='gray.500'>
							Año de admisión: <b>{admission_year}</b>
						</Text>
						<Text color='gray.500'>
							Ciclo actual: <b>{current_cycle}</b>
						</Text>
						<Text color='gray.500'>
							Inicio de programa: <b>{program_start_date}</b>
						</Text>
					</SimpleGrid>
				</Card.Root>
