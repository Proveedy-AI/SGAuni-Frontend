...
export const EvaluationSummaryTable = ({ data }) => {
	...
	// Helper para renderizar tabla de evaluaciones
	const renderEvaluationsTable = (evaluations, hasConfiguratedWithWeight) => (
		<Table.Root>
			<Table.Header>
						...
						>
							...
							{hasConfiguratedWithWeight && (
								<Table.Cell textAlign='center' borderRightColor={borderColor}>
									{evaluation.weight_percentage !== null && evaluation.weight_percentage !== undefined
	...
	// Tabla principal (course_info)
	const mainHasWeight = data?.evaluations?.some(e => e.weight_percentage !== null);

	// Render
	return (
		<>
			{/* Tabla principal */}
			<Card.Root bg={bgColor} borderColor={borderColor} mb={6}>
				<Card.Header>
					<Flex direction='column' align='start' gap={1}>
						<Text fontWeight='bold' color='blue.700'>
							{data?.course_info?.professor_name || '-'}
						</Text>
						<Text fontSize='sm' color='gray.600'>
							{data?.course_info?.type_schedule || '-'}
						</Text>
					</Flex>
				</Card.Header>
				<Card.Body p={3}>
					<Box overflowX='auto'>
						<TableScrollArea>
							{renderEvaluationsTable(data?.evaluations, mainHasWeight)}
						</TableScrollArea>
					</Box>
				</Card.Body>
			</Card.Root>

			{/* Tablas de otros profesores */}
					...
					return (
						<Card.Root bg={bgColor} borderColor={borderColor} mb={6} key={`prof-table-${prof.professor_name}-${idx}`}>
							...
							<Card.Body p={3}>
								<Box overflowX='auto'>
									<TableScrollArea>
										{renderEvaluationsTable(prof.evaluations, hasWeight)}
									</TableScrollArea>
								</Box>
							</Card.Body>
						</Card.Root>
...
EvaluationSummaryTable.propTypes = {
...
export const MyEvaluationsByCourseView = () => {
	...
	const { data: dataMyEvaluations, isLoading: isLoadingMyEvaluations } =
	...
	// Sumar evaluaciones de data.evaluations y de other_professors
	const mainEvals = data?.evaluations || [];
	...
	const pendingEvaluations = allEvals.filter(
		(e) => e.grade_obtained === null || e.grade_obtained === undefined
	).length;
		...
		<Box maxW='full' mx='auto'>
			...
			<Card.Root bg='blue.50' p={6} borderRadius='xl'>
					...
					<HStack mt={2} spacing={4}>
						...
						>
							...
							<Text as='span' fontWeight='bold' ml={1}>
								{data.course_info?.group_section}
							</Text>
						</Badge>
						<Badge
							bg='white'
					...
					>
						...
						<Flex justify='space-between' align='center'>
							...
							<Text fontSize='2xl' fontWeight='bold' color='blue.600'>
								{data?.shared_final_grade ?? data?.final_grade}
							</Text>
						</Flex>
					</Flex>
					<Flex
              ...
              <Text
								...
								fontSize='xl'
								fontWeight='bold'
								color={data?.shared_final_grade >= 10.5 ? 'blue.700' : 'red.600'}
							>
							...
							<Text
								...
								fontSize='xl'
								fontWeight='bold'
								color={data?.final_grade >= 10.5 ? 'blue.700' : 'red.600'}
							>
			...
			<Card.Root bg={bgColor} borderColor={borderColor}>
				<Card.Body p={3}>
					<EvaluationSummaryTable data={data} />
				</Card.Body>
			</Card.Root>
		</Box>
	);
